...
let proportionChart = null; // 用于存放echart初始化返回的实例
const Data = () => {
    ...
    const setPieChart = (data) => {
        if (!window.echarts)
            return;
        const echar = (window ? window.echarts : null);
        if (window && echar) {
            console.log(data);
            // 初始化 饼图返回实例
    ...
    return (<div className={CSS.data}>
      ...
      <div className={CSS.structure}>
        ...
        <div className={CSS.content}>
          {(totalType == 'expense' ? expenseData : incomeData).map((item) => (<div className={CSS.item} key={item.type_id}>
                ...
                <div className={CSS.right}>
                  <div className={CSS.percent}>
                    <Progress shape="line" percent={Number((item.number /
