...
const BillItem = ({ bill }) => {
    ...
    return (<div className={CSS.item}>
      ...
      {bill &&
            bill.bills.map((item) => (<Cell className={CSS.bill} key={item.id} onClick={() => goToDetail(item)} title={<>
                <CustomIcon type={item.type_id
                        ? typeMap[item.type_id].icon
                        : null}/>
                <span className={CSS.typeName}>{item.type_name}</span>
              </>} description={<span style={{ color: item.pay_type === 2 ? 'red' : '#39be77' }}>{`${item.pay_type === 1 ? '-' : '+'}${item.amount}`}</span>} help={<div>
