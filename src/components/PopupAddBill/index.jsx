        ...
        <div className={CSS.addWrap}>
          ...
          <div className={CSS.typeWrap}>
            <div className={CSS.typeBody}>
              {(payType === 'expense' ? expense : income).map((item) => (<div className={CSS.typeItem} key={item.id} onClick={() => choseType(item)}>
                  <span className={cx({
                [CSS.iconfont]: true,
          ...
          <div className={CSS.remark}>
            {showRemark ? (<Input autoHeight showLength maxLength={50} type="text" rows={3} value={remark} placeholder="请输入备注信息" onChange={(val) => setRemark(val)} onBlur={() => setShowRemark(false)}/>) : (<span onClick={() => setShowRemark(true)}>
                {remark || '添加备注信息'}
              </span>)}
          </div>
